  ...
  return (
    <div className="notes" >
        ...
        <form onSubmit={handleSubmit(submit)}>
            ...
            {errors.title && <p className="p1" style={{marginLeft:'330px',marginTop:'-25px',color:'red'}}> {errors.title.message}</p> }
            <textarea  type="text" placeholder="Add description" {...register("description", { required: "Required!" })}></textarea>
            {errors.description && <p  className="p2" style={{marginLeft:'330px',marginTop:'-90px',color:'red'}}> {errors.description.message}</p>}
